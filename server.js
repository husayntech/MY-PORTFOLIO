...
function dbRun(sql, params = []) {
  // sql.js throws on undefined params; null binds cleanly and COALESCE treats it as "no change"
...
// Add a column to an existing table if it's missing (migration for old DB files)
...
async function startServer() {
  ...
  if (fs.existsSync(DB_PATH)) {
    ...
    db = new SQL.Database();
    // Run initialization script inline
    require('./scripts/init-db.js');
    // The init script creates and saves the DB, so reload it
    const fileBuffer = fs.readFileSync(DB_PATH);
    db = new SQL.Database(fileBuffer);
  }
  ...
  // Insert default site_config rows that newer versions of the app expect
  function ensureConfigDefault(key, value) {
    ...
    if (!existing) {
      ...
      console.log(`✅ Migration: added config default ${key}`);
    }
  }
  ...
  // Serve the HTML pages with the saved styles + content injected (no flash on refresh)
  const PAGE_FILES = {
  ...
  // Serve other public assets (e.g. the shared /editor.js and /editor.css) at root
  ...
  app.get('/api/sections', (req, res) => {
    const sections = dbAll('SELECT * FROM sections WHERE is_visible = 1 ORDER BY sort_order');
    res.json(sections);
  });
  ...
  app.get('/api/projects', (req, res) => {
    const projects = dbAll('SELECT * FROM projects WHERE is_visible = 1 ORDER BY sort_order');
    res.json(projects);
  });
  ...
  app.get('/api/skills', (req, res) => {
    ...
    let skills;
    if (category) {
      skills = dbAll('SELECT * FROM skills WHERE category = ? AND is_visible = 1 ORDER BY sort_order', [category]);
    } else {
      skills = dbAll('SELECT * FROM skills WHERE is_visible = 1 ORDER BY sort_order');
    }
    res.json(skills);
  });
  ...
  app.get('/api/services', (req, res) => {
    const services = dbAll('SELECT * FROM services WHERE is_visible = 1 ORDER BY sort_order');
    res.json(services);
  });
  ...
  app.get('/api/testimonials', (req, res) => {
    const testimonials = dbAll('SELECT * FROM testimonials WHERE is_visible = 1 ORDER BY sort_order, created_at DESC');
  ...
  // Public so the on-page style editor can persist without a login
  app.put('/api/styles', (req, res) => {
    const styles = req.body;
    for (const [key, value] of Object.entries(styles)) {
      // Upsert so new style keys can be added without schema changes
      ...
      dbRun("UPDATE style_settings SET setting_value = ?, updated_at = datetime('now') WHERE setting_key = ?", [String(value), key]);
    }
    res.json({ success: true });
  });
  ...
  // Public so the on-page style editor can persist content without a login
  app.put('/api/config', (req, res) => {
    const configs = req.body;
    for (const [key, value] of Object.entries(configs)) {
      // Upsert so new content keys can be added without schema changes
  ...
  // ============ PAGE ROUTES ============
  // (The HTML pages themselves are served by the middleware above, which injects
  // the saved styles + content so there's no flash on refresh.)
